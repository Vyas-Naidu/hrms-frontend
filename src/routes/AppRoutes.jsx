...
function HRPage({ Component, title }) {
  ...
  return (
    <div className={layoutStyles["app-layout"]}>

      <Sidebar open={sidebarOpen} onClose={() => setSidebarOpen(false)} />
      {sidebarOpen && (
        <button
          className={layoutStyles["sidebar-backdrop"]}
          aria-label="Close navigation"
          onClick={() => setSidebarOpen(false)}
        />
      )}

      <div className={layoutStyles["main-area"]}>

        <Navbar title={title} onMenuClick={() => setSidebarOpen((open) => !open)} />

        <main className={layoutStyles["page-content"]}>
          <Component />
        </main>
  ...
  return (
    <BrowserRouter>
      <Routes>
        ...
        {/* Email */}
        <Route path="/hr/email" element={<Email />} />
